...
export default function ManiKunjHeader(){
  const cart = useUniqloStore(s=> (s as any).cart);
  const wishlist = useUniqloStore(s=> (s as any).wishlist);
  const nav = useUniqloStore(s=> (s as any).navigation);
  const siteSettings = useUniqloStore(s=> (s as any).siteSettings);
