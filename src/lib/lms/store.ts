  ...
  persist(
    (set,get)=>({
      ...
      enroll:(courseId)=>{
        ...
        if(exists) return;
        set(s=>({ enrollments:[...s.enrollments, {userId:u.id, courseId, progress:0, completedLessonIds:[], enrolledAt: Date.now()}], users: s.users.map(x=> x.id===u.id?{...x, enrolledCourseIds:[...x.enrolledCourseIds, courseId]}:x), currentUser: s.currentUser?{...s.currentUser, enrolledCourseIds:[...s.currentUser.enrolledCourseIds, courseId]}:null }));
      },
      enrollPackage:(pkgId)=>{
